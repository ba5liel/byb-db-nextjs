...
import {
  Plus, Edit, Trash2, Eye, Search, Filter, ChevronLeft, ChevronRight,
  Users, UserCheck, TrendingUp, X, AlertCircle, RotateCcw, UserMinus, Upload,
...
import { useMembers, useServices, useDeleteMember, useRestoreMember, useCreateMember } from "@/lib/api/hooks"
import { useLanguage } from "@/lib/language-context"
import { getTranslation } from "@/lib/translations"
import { toast } from "@/hooks/use-toast"
import type { MemberDto, SubCommunity, Sex, MaritalStatus, MemberStatus, GroupType, AgeGroup, CreateMemberDto } from "@/lib/api/types"
import { membersService } from "@/lib/api/services"
import { ExportButton } from "@/components/import-export/export-button"
import { ImportDialog } from "@/components/import-export/import-dialog"
import { MEMBER_EXPORT_COLUMNS } from "@/lib/utils/import-export"

const FORMER_STATUS_LABELS: Record<string, { label: string; color: string }> = {
  removed: { label: "Removed", color: "bg-red-500/10 text-red-500" },
  transferred: { label: "Transferred Out", color: "bg-blue-500/10 text-blue-500" },
  deceased: { label: "Deceased", color: "bg-gray-500/10 text-gray-400" },
}

const STATUS_ICON: Record<string, React.ReactNode> = {
  active:   <CheckCircle2 className="w-4 h-4 text-green-500" />,
  inactive: <MinusCircle  className="w-4 h-4 text-yellow-500" />,
  removed:  <XCircle      className="w-4 h-4 text-red-500" />,
}
...
export default function MembersPage() {
  ...
  const [formerStatus, setFormerStatus] = useState<MemberStatus>("removed")
  const [formerPage, setFormerPage] = useState(1)
  const [restoreDialogOpen, setRestoreDialogOpen] = useState(false)
  const [memberToRestore, setMemberToRestore] = useState<{ id: string; name: string } | null>(null)
  ...
  const { data: formerData, isLoading: formerLoading } = useMembers({
    page: formerPage,
    limit: 20,
    search: formerSearch || undefined,
    memberStatus: formerStatus,
  })
  ...
  const formerMembers = formerData?.data || []
  const formerPagination = formerData?.pagination
  ...
  async function handleImportRow(dto: CreateMemberDto) {
  ...
  async function handleRestoreConfirm() {
    ...
    try {
      await restoreMutation.mutateAsync(memberToRestore.id)
      toast({ title: "Restored", description: `${memberToRestore.name} has been restored to active.` })
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "active": return <Badge className="bg-green-500">{t.members.active}</Badge>
      case "inactive": return <Badge variant="secondary">{t.members.inactive}</Badge>
      case "removed": return <Badge variant="destructive">{t.members.removed}</Badge>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <ExportButton
            ...
            filename="members"
            title="BYB Church — Member Directory"
          />
      ...
      <Tabs defaultValue="active" className="space-y-6">
        <TabsList className="grid w-full max-w-xs grid-cols-2">
          ...
          <TabsTrigger value="former" className="gap-2">
        ...
        <TabsContent value="active" className="space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div>
                    ...
                    <Select value={statusFilter} onValueChange={setStatusFilter}>
                      <SelectTrigger id="status-filter"><SelectValue placeholder={t.members.selectStatus} /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select value={genderFilter} onValueChange={setGenderFilter}>
                      <SelectTrigger id="gender-filter"><SelectValue placeholder={t.members.selectGender} /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select value={subCommunityFilter} onValueChange={setSubCommunityFilter}>
                      <SelectTrigger id="subcommunity-filter"><SelectValue placeholder={t.members.selectSubCommunity} /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select value={ageGroupFilter} onValueChange={setAgeGroupFilter}>
                      <SelectTrigger id="age-group-filter"><SelectValue placeholder={t.members.selectAgeGroup} /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select value={groupTypeFilter} onValueChange={setGroupTypeFilter}>
                      <SelectTrigger id="group-type-filter"><SelectValue placeholder={t.members.selectGroupType} /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select value={maritalStatusFilter} onValueChange={setMaritalStatusFilter}>
                      <SelectTrigger id="marital-status-filter"><SelectValue placeholder={t.members.selectMaritalStatus} /></SelectTrigger>
                      <SelectContent>
                ...
                <div className="flex justify-end pt-4">
                  <Button
                    variant="outline"
                    onClick={() => {
                      setStatusFilter("all"); setGenderFilter("all"); setAgeGroupFilter("all")
                      setSubCommunityFilter("all"); setGroupTypeFilter("all"); setMaritalStatusFilter("all")
                    }}
                  >
          ...
          <Card variant="glass">
            <CardHeader>
              ...
              <CardDescription>
                {pagination && `Showing ${((pagination.page - 1) * pagination.limit) + 1}–${Math.min(pagination.page * pagination.limit, pagination.total)} of ${pagination.total} members`}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <Link href="/members/new">
                    <Button><Plus className="w-4 h-4 mr-2" />Add First Member</Button>
                  </Link>
                </div>
              ) : (
                <div className="space-y-4">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative w-12 h-12">
                          ...
                          <span className="absolute -bottom-0.5 -right-0.5 bg-background rounded-full">
                            {STATUS_ICON[member.memberStatus || "active"] ?? STATUS_ICON.active}
                          </span>
                        </div>
                        <div>
          ...
          {pagination && pagination.pages > 1 && (
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" size="sm" onClick={() => setCurrentPage(p => p + 1)} disabled={currentPage === pagination.pages}>
        ...
        <TabsContent value="former" className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    value={formerSearch}
                    onChange={(e) => { setFormerSearch(e.target.value); setFormerPage(1) }}
                    className="pl-10 h-12"
                  />
                </div>
                <Select value={formerStatus} onValueChange={(v) => { setFormerStatus(v as MemberStatus); setFormerPage(1) }}>
          ...
          <Card variant="glass">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {FORMER_STATUS_LABELS[formerStatus]?.label ?? "Former"} Members
              </CardTitle>
              <CardDescription>
                {formerPagination
                  ? `${formerPagination.total} member${formerPagination.total !== 1 ? "s" : ""} found`
                  : "Loading..."}
                {formerStatus === "removed" && " — Removed members can be restored to active."}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {formerLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">
                    No {FORMER_STATUS_LABELS[formerStatus]?.label.toLowerCase()} members
                  </h3>
                  <p className="text-muted-foreground">
                    {formerStatus === "removed" ? "No members have been removed." :
                     formerStatus === "transferred" ? "No members have been transferred out." :
                     "No deceased member records found."}
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {(formerMembers as MemberDto[]).map((member) => {
                    const statusMeta = FORMER_STATUS_LABELS[member.memberStatus] ?? { label: member.memberStatus, color: "bg-white/10 text-foreground" }
                    return (
                          ...
                          <div className="flex gap-2">
                            ...
                            {member.memberStatus === "removed" && (
                              <Button
                                ...
                                onClick={() => handleRestoreClick(member._id, member.fullName)}
          ...
          {/* Former pagination */}
          {formerPagination && formerPagination.pages > 1 && (
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Page {formerPagination.page} of {formerPagination.pages}
              </p>
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => setFormerPage(p => p - 1)} disabled={formerPage === 1}>
                  <ChevronLeft className="w-4 h-4 mr-1" />Previous
                </Button>
                <Button variant="outline" size="sm" onClick={() => setFormerPage(p => p + 1)} disabled={formerPage === formerPagination.pages}>
                  Next<ChevronRight className="w-4 h-4 ml-1" />
                </Button>
              </div>
            </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Restore <span className="font-semibold">{memberToRestore?.name}</span> to active membership?
              Their record will be fully reinstated.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
